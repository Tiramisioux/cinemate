...
def load_settings(filename: str | Path) -> dict:
    ...
    # ── Pico Wi-Fi link defaults ─────────────────────────────────────────
    pico_defaults = {
        "enabled": False,
        "bind_host": "0.0.0.0",
        "beacon_host": "192.168.4.255",
        "beacon_port": 50020,
        "command_port": 50021,
        "time_beacon_hz": 10,
        "state_broadcast_on_change": True,
        ...
        "allow_cli_commands": True,
        "allow_method_calls": True,
        "cli_prefix_whitelist": [],
        "state_keys": ["iso", "shutter_a", "fps", "wb", "is_recording"],
    }
    pico_cfg = settings.setdefault("pico_link", {})
    for k, v in pico_defaults.items():
        pico_cfg.setdefault(k, v)
    settings["pico_link"] = pico_cfg
