...
> (outside the repo, per the external-workspace convention), committed so agent
> sessions can read it. The ledger entry is **C0** (formerly B14) in [`PLAN.md`](PLAN.md) in this
> directory (moved 2026-08-25 from the review ledger to the dev track). Once the
> feature merges to `dev`, the code is the truth and both copies are historical.
