# Kickoff prompt — the pane session (C9.1–C9.5)

> **Second of two. Do not start this until Phase 0 has landed and G10/G11 have reported** — see
> [`SONNET-PROMPT.md`](SONNET-PROMPT.md) beside this file. Phase 0 puts an embedded thumbnail in
> the DNG, which is the path this pane serves by default; the raw decoding this session works on
> is the **fallback**, for footage shot before that change. Running this first is not wrong, just
> premature: you would be tuning the slow path before the fast one exists.
...
One addition the plan calls for and this prompt's five commits do not cover: the frame route
branches on whether the take carries an embedded thumbnail, and **the HUD must say which path a
frame came from** (open decision 8). Build the branch point in C9.3 as a single seam with a clear
name, even though the thumbnail side stays unimplemented until Phase 0 lands — do not scatter the
decision through the route.
