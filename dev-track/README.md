...
| C9 | Restore 12-bit ClearHDR **with working CCMP tone mapping** — the target is the mid-August state (cinepi-raw `4b9c9f6`), not the 08-27 mono milestone; plus the start-up failure, the pedestal fill, and why it keeps breaking | **Filed 2026-08-31, retargeted same day**; two start-up fixes pushed, the tone-mapping regression is analysis only; gate GT and G0/GR/G1–G6 unrun | `C9-clearhdr-12bit-restore/PLAN.md` (ledger entry incl. gates, hypotheses, why-it-breaks and prevention); measurement provenance on cinemate branch `docs/ccmp12-workspace` @ `21f1f1b` (`innomaker585/ccmp12-lut/`) | `claude/cinemate-12bit-hdr-research-t7ylrd` off `dev` (cinemate only; the cinepi-raw-side items in §R and §S are unpushed — read-only access) |
