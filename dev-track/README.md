...
|---|---|---|---|---|
| C0 | Format drive from the settings editor's RAW pane | **DONE** — merged (PR #152, `e54e691b`) and Pi-verified 2026-08-26, full destructive checklist passed | `C0-format-drive/PLAN.md` (ledger entry) + `FORMAT-DRIVE-PLAN.md` (spec, now historical) + `SONNET-PROMPT.md` (kickoff prompt, spent) | `feature/raw-pane-format-drive` — merged to `dev` |
...
| C9 | Clip playback — review a recorded take in the web GUI, at the conform frame rate | **Thumbnail-first as of 2026-09-01** (operator steer), with the raw decoder kept as the fallback for footage shot before the change. Verified against `cinepi-raw` `774402c`: the DNG carries **no** thumbnail today — `dng_encoder.cpp:781-785` assigns five `thumb*` fields nothing reads, and the lores buffer reaches `dng_save` as `[[maybe_unused]]` — but the lores frame already arrives per frame at 1272×720, so Phase 0 writes what is already there. Pane written and desk-verified 2026-08-27; plan re-grounded 2026-09-01 (nine claims corrected, six defects found). **Now a two-repo step** needing a cinepi-raw build. All twelve gates unrun; G10/G11 (does the thumbnail land, what does it cost to write) now decide the feature, and G1/G2 are demoted to fallback-quality gates | `C9-clip-playback/PLAN.md` (ledger entry) + `GATES.md` (twelve gates in five sessions, predictions stated in advance) + `SONNET-PROMPT.md` (Phase 0 — the thumbnail, unspent) + `SONNET-PROMPT-PANE.md` (the pane session, unspent, runs after G10/G11); feasibility study, design spec, harness and mockup in external `development/playback-pane/` | `feature/clip-playback` off `dev` @ `714ef7b4` (cinemate, 2 commits, **not pushed**, 1381 behind `dev`) + a cinepi-raw branch for Phase 0, **to be cut** |
