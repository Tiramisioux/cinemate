...
| C9 | Restore working 12-bit ClearHDR **on `dev`** — the probe-failure regression since the milestone tag, the deployment ref, the three fixed-on-dev defects, the boot-latched pedestal fill, and four regression traps | **Filed 2026-08-31**; one fix pushed (probe-failure diagnostic + tests), gates G0/GR/G1–G6 unrun | `C9-clearhdr-12bit-restore/PLAN.md` (ledger entry incl. gates, hypotheses and scope record); curve provenance on cinemate branch `docs/ccmp12-workspace` @ `21f1f1b` (`innomaker585/ccmp12-lut/`) | `claude/cinemate-12bit-hdr-research-t7ylrd` off `dev` (cinemate only; the cinepi-raw-side recommendations in §R are unpushed — read-only access) |
