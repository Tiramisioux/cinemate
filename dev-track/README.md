...
| C9 | Clip playback — review a recorded take in the web GUI, at the conform frame rate, decoded from the CinemaDNG on the card | **First increment written and desk-verified 2026-08-27; plan re-grounded against the repo 2026-09-01** — fifth settings-editor tab, decoder + endpoints + 9 tests, verified through the real blueprint against real DNGs. The re-grounding found six defects the desk harness could not see, corrected nine claims, and downgraded the `conform_frame_rate` docs fix from done to **unpushed**; whether the decoder applies the `LinearizationTable` is **not established** (a log take would render black). **All ten hardware gates unrun**, and G1 (decode throughput) + G2 (storage read bandwidth) gate everything after them | `C9-clip-playback/PLAN.md` (ledger entry incl. measurements, the two silent bugs found by running it, the six found by re-grounding it, and open decisions) + `GATES.md` (ten gates in four sessions, predictions stated in advance) + `SONNET-PROMPT.md` (kickoff prompt, unspent); feasibility study, design spec, harness and mockup in external `development/playback-pane/` | `feature/clip-playback` off `dev` @ `714ef7b4` (cinemate only, cut 2026-08-27, 2 commits, **not pushed**, 1381 commits behind `dev`) |
