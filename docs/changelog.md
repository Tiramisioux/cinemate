...
- **A fourth embedded-thumbnail type: colour JPEG** — `image_capture.thumbnail` (and `set thumbnail`) now takes the words `off` / `mono` / `colour` / `jpeg` (the legacy `0`-`3` still work), where `jpeg` is a baseline JPEG, quality 85, 4:2:0 — measured 3–8 KB per frame at the shipped 320×180 default and 9–16 KB at 640×360, against 230,400 B / 57,600 B for mono at the same sizes, paid in the most CPU of the four (a YUV→RGB conversion plus the JPEG encode itself). Colour stays the default; `jpeg` is opt-in. See [settings.jsonc's "DNG thumbnails" section](settings-json.md#dng-thumbnails) for the full table.
