...
- **Saving from the settings editor no longer deletes the settings it has no field for** — the page built the whole saved document out of its own controls, so every key without one (`input_peripherals.pots`, `system.https.*`, `system.web_api.*`, the `arrays.hdr_*` tables, `image_capture.hdr.iso_max`, `output_peripherals.oled.width`/`height`, `sensors.raw_buffer_count`, `$schema`, and the rest of "[Settings with no field on this page](settings-json.md#settings-with-no-field-on-this-page)") was dropped from `settings.jsonc` and left to CineMate's built-in defaults. A save now merges the page's values over the file on disk, on the server as well as in the browser, so an unrendered key keeps what it said. This is what cost a camera its `image_capture.hdr.imx585_clear_hdr_12bit` on 2026-09-15 and switched 12-bit ClearHDR back on: a missing key there means "on". The lists the page does build in full — GPIO in/out, quad rotary encoders, per-mode fps ceilings, value steps — still replace what they replace, so deleting an entry still deletes it. Also: an explicit `input_peripherals.quad_rotary_controller.enabled: false` is no longer flipped back to `true` by an unrelated save.

- **ClearHDR is two switches now, one per bit depth, and the resolution list decides the rest** — the settings editor offers **Enable IMX585 ClearHDR 16-bit** and **Enable IMX585 ClearHDR 12-bit** (`image_capture.hdr.imx585_clear_hdr_16bit` / `_12bit`), and **Resolutions offered** then decides which frame sizes of each you get, exactly as it already did for SDR. The third switch, `imx585_clear_hdr_16bit_hd`, is gone: it gated *every* binned ClearHDR mode regardless of bit depth, so turning it on for the 16-bit HD mode silently handed back 12-bit HD as well — which is what its own help text promised it would not do. Defaults are unchanged in effect for a stock camera (16-bit on, 12-bit off, all four K categories on), which is the five-mode imx585 table the [sensors page](sensors.md#imx585-starlight-eye) documents. See [ClearHDR](clear-hdr.md#which-clearhdr-modes-you-are-offered).
