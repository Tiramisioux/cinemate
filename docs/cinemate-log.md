...
**IMX585 and IMX283 only.** Support is decided by the sensor's black level, not chosen: the two shipped log DNG specs are built for BlackLevel 3200, which is what those two report and no other sensor does.
...
It toggles log on and off at the live mode's default target, and restarts the camera to apply it. Press it again to go back to linear.
...
Forcing a target only matters on a 16-bit ClearHDR source, where both are valid. `12` is the default and `10` trades a little more compression for a smaller file. A 12-bit source has only one valid target, `10`, so forcing `12` there records plain linear DNGs instead. That is not an error, and the badge tells you which way it went.

**Save changes** writes the file and restarts CineMate.

### From the terminal (CineMate CLI)
