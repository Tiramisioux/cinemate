...
Both the take-strip preview cards and the player itself decode this embedded thumbnail rather than the raw frame — raw decode is far more demanding on the Pi. It plays **in colour, at 320×180, by default**: `image_capture.thumbnail` defaults to `2` (colour) and `image_capture.thumbnail_size` defaults to `2` (quarter of the lores plane) — an operator decision made 2026-09-13, since colour at quarter size (172,800 B/frame) costs fewer bytes than mono did at half size. `set thumbnail 1` (or the same key set to `1` in the settings file) switches to mono, at a third of the bytes per frame. `image_capture.thumbnail_size` trades resolution for size independently of mode — `0` is the full lores plane, `1` is half. Changing `thumbnail_size` restarts the camera; changing `thumbnail` takes effect on the next take, with no restart.
