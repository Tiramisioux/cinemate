...
| Key | Written by | Description | Safe to change manually? |
|-----|------------|-------------|--------------------------|
| dynamic_resolution_enabled | CineMate | Toggle for automatic FPS-driven resolution downgrade/upgrade (`set dynamic resolution`); persisted and read back at startup. When unset, the startup value comes from `image_capture.dynamic_resolution` in `settings.jsonc` (on by default) | Yes |
...
| dynamic_resolution_priority | CineMate | Which axis of quality the substitution gives up first once the desired mode's own class runs out: `mode` (hold bit depth + ClearHDR, drop resolution), `resolution` (hold frame size, drop class), `none` (never leave the class). Set with `set dynamic resolution priority`; persisted and read back at startup. When unset, the startup value comes from `image_capture.dynamic_resolution_priority` in `settings.jsonc` (`mode` by default) | Yes |
...
| thumbnail | CineMate -> CinePi-raw | Embedded DNG thumbnail mode: `0` off, `1` mono, `2` colour, `3` colour JPEG (default). `image_capture.thumbnail` speaks the words (`off`/`mono`/`colour`/`jpeg`) in settings.jsonc; this wire value is always the int -- cinemate translates at boot and at `set thumbnail`. Applied live (no camera restart), written per frame into new takes only. See [settings.jsonc](settings-json.md#dng-thumbnails) for the bytes/CPU cost of each. | Yes (publish key to apply) |
| thumbnail_size | CineMate -> CinePi-raw | Right-shift downscale of the thumbnail plane, every mode including jpeg; `0` is the full lores size. Seeded at boot from `image_capture.thumbnail_size` (settings.jsonc), default `1` (half, 640×360). Changing it restarts the camera; a settings-editor field exists (Resolution & sensor -> DNG thumbnails), a Save away from taking effect, but no live CLI verb | No (seeded from settings at every boot to avoid a stale pre-Phase-0 resident value collapsing the thumbnail; change by hand only if you understand the restart) |
