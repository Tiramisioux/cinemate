...
| thumbnail_size | CineMate -> CinePi-raw | Right-shift downscale of the thumbnail plane; `0` is the full lores size. Seeded at boot from `image_capture.thumbnail_size` (settings.jsonc), default `2` (quarter, 320×180). Changing it restarts the camera; no CLI verb or settings-editor field yet | No (seeded from settings at every boot to avoid a stale pre-Phase-0 resident value collapsing the thumbnail; change by hand only if you understand the restart) |
