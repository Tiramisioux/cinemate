...
2. Writes your changes. If only values changed, the surgical writer keeps every comment, the key order and the file's formatting. If a key was added or removed (a structural change), it falls back to a full rewrite, which is correct but drops every comment.
    ...
    !!! note "A save now merges into the file instead of replacing it"
        Until this release the saved document was built from the form's own fields alone, so **any setting the page had no field for was not written back at all** — it was dropped from `settings.jsonc` entirely. Observed on a camera 2026-09-13: one save removed the whole `system.https` and `system.web_api` blocks, blanked both cameras' `tuning_file_override.path`, emptied `input_peripherals.pots` and cleared `sensors.cam1.camera_name`. Nothing warned, and the camera kept running, because CineMate falls back to its built-in defaults for whatever is missing — so the loss stayed invisible until you read the file, or until a default differed from what you had set. On 2026-09-15 that is exactly what happened: a camera lost `image_capture.hdr.imx585_clear_hdr_12bit`, whose absence means "on", and 12-bit ClearHDR came back by itself.

        A save now writes the values the page carries **over** the file on disk, so a key with no field on the page keeps whatever it said. The page still owns what it builds in full: the GPIO in/out lists, the quad rotary's encoders, the per-mode fps ceilings and every value-steps list — removing an entry there still removes it. See "[Settings with no field on this page](settings-json.md#settings-with-no-field-on-this-page)" for what that now protects.

        Comments are a separate matter and are still lost on a structural save, as above. `diff .settings-backups/<newest> settings.jsonc` after a save remains the way to see exactly what changed.
