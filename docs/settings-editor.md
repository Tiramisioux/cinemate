...
| Exposure & steps | Value steps (click-stops per control) · Resolution & sensor (resolutions, bit depths, dynamic resolution, ClearHDR startup values) · Per-mode fps ceilings (per-sensor-mode overrides — see `custom_modes` in the settings reference) |
