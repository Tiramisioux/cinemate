...
* **enabled** – set to `true` to start the hotspot automatically on boot.

## 2. `geometry`

Controls image orientation for each camera port (`cam0`, `cam1`, etc.). Example:
...
These settings let you mount cameras in any orientation and still get an upright preview and recording.
...
Preset lists for exposure and frame‑rate settings. Cinemate will step through these values unless you enable free mode.
...
* **conform_frame_rate** – frame rate used when conforming footage in post.
...
## 14. `quad_rotary_encoders`

Support for the Adafruit RGB rotary encoder breakout with four dials. Each entry assigns a dial to a setting and clones the behaviour of a button pin.
...
Configuration for the optional OLED status screen.
...
## 16. `welcome_message` and `welcome_image`
