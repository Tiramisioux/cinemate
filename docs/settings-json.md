...
![Grove HAT potentiometers section of the CineMate settings editor](images/gui-pots.png)
...
| `image_capture.hdr.iso_max` | Highest ISO offered while a ClearHDR mode is engaged; default `1585`, `null` lifts it. Applies **only** in ClearHDR modes; SDR is never capped. The driver caps analogue gain at code 80, which the ISO steps reach at ~1585 — measured, ISO 1600/2500/3200 all land on code 80 and record the same exposure, so above the cap only the preview brightens (ISP digital gain) and exposure judged from the monitor is wrong. The 1600 step is kept and lands on 1585, shown green in both GUIs to say the camera is holding it; 2500 and 3200 are dropped. |
