...
class ParameterKey(Enum):
    ...
    THUMBNAIL_SIZE    = "thumbnail_size"  # right-shift downscale of the thumbnail plane, 0 = full lores size, default 2 = quarter (cinepi-raw CONTROL_KEY_THUMBNAIL_SIZE; changing it restarts the camera). Settings owner: image_capture.thumbnail_size (thumbnail_size_startup_value(), config_loader.py). Not exposed via CLI/settings-editor — settings.jsonc only, since a live change restarts the camera; still seeded unconditionally at boot so a resident pre-Phase-0 value (PI-008) doesn't collapse the thumbnail (see cinepi-raw's sync() guard, C-2)
    IR_FILTER         = "ir_filter"
