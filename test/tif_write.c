...
// Modified TIFFAppendToStrip to allow direct writing
// Updated TIFFAppendToStrip function
static int
TIFFAppendToStrip(TIFF* tif, uint32_t strip, uint8_t* data, tmsize_t cc)
{
    ...
    uint64_t stripbytes64;
    uint64_t stripoff64;

    // Use td_stripoffset_p instead of td_stripoffset
    if (!SeekOK(tif, td->td_stripoffset_p[strip] + tif->tif_curoff))
        return (0);

    stripbytes64 = TIFFGetStrileByteCount(tif, strip);
    if (stripbytes64 == 0 || tif->tif_curoff == 0)
    {
        bytecount64 = cc;
    }
    else
    {
        bytecount64 = stripbytes64 + cc;
    }

    stripoff64 = TIFFGetStrileOffset(tif, strip);
    if (!(tif->tif_flags&TIFF_DIRTYSTRIP))
    {
        // Use TIFFSetField instead of tif_setget
        TIFFSetField(tif, TIFFTAG_STRIPBYTECOUNTS, bytecount64);
        TIFFSetField(tif, TIFFTAG_STRIPOFFSETS, stripoff64);
        tif->tif_flags |= TIFF_DIRTYSTRIP;
    }

    // Write data directly if it's not in the internal buffer
    if (data != tif->tif_rawdata)
    {
    ...
    {
        if (!WriteOK(tif, tif->tif_rawdata, cc))
        {
            TIFFErrorExt(tif->tif_clientdata, module,
                         "Write error at scanline %lu",
                         (unsigned long) tif->tif_row);
