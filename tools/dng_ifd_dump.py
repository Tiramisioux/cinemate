...
strip size is the per-frame cost of that thumbnail.
...
Standard library only. Cross-check with `exiftool -a -G1 <file>`.
...
TAG_STRIP_BYTES, TAG_SOFTWARE, TAG_UCM, TAG_LIN_TABLE = 279, 305, 0xC614, 0xC618
    ...
    with open(path, "rb") as handle:
        ...
        while next_ifd and index < 8:
            ...
            line = (f"  IFD{index}: subfile={get(TAG_SUBFILE)} {get(TAG_WIDTH)}x{get(TAG_HEIGHT)} "
                    ...
                    f"compression={get(TAG_COMPRESSION)} strip={strip:,} B  {lin_text}")
            if index == 0:
